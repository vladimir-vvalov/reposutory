{% macro tbmacro_validate_config() -%}
  ...
  {%- endset %}
  {% set invalid_preset_merge_small_msg -%}
  ...
  {%- set cast_columns = raw_tbm_cast_columns if tbm_contract else false -%}
  ...
  {%- set tbm_contract = tbmacro.tbmacro_validate_contract(raw_tbm_contract, model) -%}
  ...
  {#-- presets --#}
  {%- if strategy == 'preset_merge_small' -%}
    {%- if unique_key is none or partition_by is not none-%}
      {% do exceptions.raise_compiler_error(invalid_preset_merge_small_msg) %}
    {%- else -%}
      {%- set mode = none -%}
...
{% macro tbmacro_validate_strategy(raw_strategy, file_format) %}
  ...
  {%- set accepted_strategies = ['append', 'merge', 'insert_overwrite', 'delete+insert', 'preset_merge_small'] -%}
...
{% macro tbmacro_validate_contract(raw_tbm_contract, model = {}) %}
  ...
  {% set no_yml_msg -%}
    Error - '{{ model.name }}' YML not found.
  ...
  {% set no_model_description_msg -%}
    Error - description for model '{{ model.name }}' YML not found.
  ...
  {% set invalid_description_msg -%}
    ...
    description not found for column: 
  {%- endset %}
  ...
  {% if model.description is none or not model.description %}
    {% do exceptions.raise_compiler_error(no_model_description_msg) %}
  {% endif %}

  {%- for key, value in model.columns.items() %}
    {% if value.description is none or not value.description  %}
