  ...
  {%- if is_existing_relation == false -%}
    ...
    {%- if tbm_config.tbm_contract == true -%}
      ...
      {%- if existing_relation.is_view or existing_relation is none -%}
        {%- if tbmacro.tbmacro_get_existing_location(file_format, spark__location()) == true -%}
          {#-- Restore table because location exists --#}
          {%- call statement('tbmacro_restore_relation', language=language) -%}
            {{ tbmacro.tbmacro_restore_relation(target_relation, spark__location()) }}
          {%- endcall -%}
      ...
      {%- call statement('create_relation', language=language) -%}
        {{ tbmacro.tbmacro_get_create_table(target_relation, existing_relation, should_full_refresh()) }}
      {%- endcall -%}
