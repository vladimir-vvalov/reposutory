{% macro tbmacro_get_create_table(relation, existing_relation, should_full_refresh) -%}
  {{ adapter.dispatch('tbmacro_get_create_table', 'tbmacro')(relation, existing_relation, should_full_refresh) }}
{%- endmacro %}

{% macro spark__tbmacro_get_create_table(relation, existing_relation, should_full_refresh) -%}
  ...
  (
    ...
    `{{ key|trim() }}` {{ value.data_type|trim() }} {{ 'not null' if value.constraints and value.constraints[0]['type'] == 'not_null' else '' }}
      comment '{{ (value.description|trim()) | replace("'", "\\'") }}'{{ ", " if not loop.last }}
    {% endfor %}
  )
  {{ file_format_clause() }}
  {{ options_clause() }}
...
{% macro tbmacro_restore_relation(relation, location) -%}
  ...
  {%- if not relation or not location -%}
    {{ exceptions.warn("Not exists relation: "~relation~" or location: "~location) }}
  {%- else -%}
...
{% macro tbmacro_cast_compiled_code(target_relation, compiled_code, tbm_config) -%}
  ...
  {%- if cast_columns == true -%}
    ...
    with _dbt__tbmacro.tbmacro_tmp_cast_{{ target_relation.identifier }} as (
      {{ compiled_code }}
    )
    select
      {% for key, value in model.columns.items() -%}
      ...
      {% endfor %}
    from _dbt__tbmacro.tbmacro_tmp_cast_{{ target_relation.identifier }}
    {%- endset %}
  {%- else -%}
